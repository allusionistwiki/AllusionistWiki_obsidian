...
- **第007話（２－３「魔女と狂犬」, pp.312–356）完結**：episodes/ch007.md・reflections/by-episode/ch007.md を新規生成。
  - 要約：アキラ覚醒→呪術医トリシーラとの対話（第五階層最高の呪術医院）→コルセスカ再会・「使い魔」提案→三報会の人質の少年・動画信仰→サイバーカラテ道場 Ｄｏｐｐｌｅｒ起動・第六階層へ落下。
  - 新キャラクターページは作らず（トリシーラは episode 疑問ノート＋用語集へ）、world/terminology への既存リンクで統合（並行源禁止）。
- **小カナ綴りバグのスキャンと修正（本次第の重点）**：
  - **壊れを発見**：ch007.md に `モロレクたちのテリトリシーラ`（テ-リ-ト-リ-シ-小ユ-ー-ラ）1カ所。raw/007 は正解の `テリトリー`（テ-リ-ト-リ-ー）。別エージェントのトリシーラ綴り修正スクリプトが過マッチで他語を破壊。
  ...
  - **修正適用**：`テリトリシーラ` → `テリトリー` を Python でバイト精密置換（encoding=utf-8）。再確認でテリトリー成立・壊れ0。
...
- **コミット**：bionic/ch007-generate ブランチへ。（次タスク：第008話 六淫操手 以降の生成）