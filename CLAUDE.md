...
│   └── <話数>_<タイトル>_[分割ID].md
├── assets/              # 画像などローカル保存
├── tools/               # 再利用ツール（親ツールのみ。JSON等は tools/data/、§8.2）
├── backlog/             # 追加機能・要望の受付箱（未着手のみ、§9）
└── wiki/                # Obsidian vault（ここに書く）
    ...
    │   │   ├── M_External/  # 実世界の神話・宗教・民話の元ネタ
    │   │   └── M_Internal/  # 作中内部で反復／転倒される神話モチーフ
    │   ├── Literature/  # 文学・思想・歴史へのオマージュ
    │   │   ├── L_External/  # 実世界的な文学作品・哲学・史実への参照
    │   │   └── L_Internal/  # 作中内部の他作品・他話への文学オマージュ
...
> ※ `terminology/` はグループフォルダ（`groups/`）と単語フォルダ（`単語（一覧）/`）の2階層構成。**用語と同様、グループも固定枠ではなく、運用中に適宜「新規作成・分割・統合・改名・再配置」する**（§3 terminology／§6）。
...
- **ページ作成のタイミング＝言及時点**（§6）：人物・組織・勢力は、地の文／台詞／固有名詞／あだ名など**原作で一度でも言及された時点でページを作成する**。情報が数行しかなくても「後でまとめて作る」をしない。既存ページがあれば作り直さず追記・改定する
...
- **グループ（`groups/`）は運用しながら育てる**：新規用語を登録するたびに既存グループの帰属を見直し、収まらないなら遠慮なく新グループを作成する。項目が膨らんだ／意味域がずれたグループは分割・統合・改名し、単語ページ側のリンクと index も同時に追随させる。重複は「主グループ1つ＋他グループからは参照」で解消（§6）。
...
- **常時再読対象**：`index.md` / `Open_Questions.md` / `Resolved.md` / `Foreshadowing/_index.md` をコンテキスト圧縮後に毎回読み直す（§8.5）。同じ謎の再登録・状態の逆行記述を防ぐ。
...
各話生成は以下の順で回す。事実ページ（characters/terminology）は**言及された時点で必ず作成／改定**し、初出・再出の別は「新規事項か追記か」の内容判定に使う（§6・並行源禁止）。

> **①本次第の着手前に必ず `wiki/status.md` で作業状況を確認する**。進捗一覧（✅/⏳ の各話ごとステータス、次タスク／話一覧の状態列）で「次に生成すべき話」と各フォルダの待機範囲を特定し、その範囲に絞って作業する。確認なく推測で着手しない。
>
> **analogies/ と mysteries/ は、コンテキスト圧縮後に毎回必ず読み直す**（§8.5）。対象＝`wiki/analogies/recursion-map.md`、`wiki/mysteries/index.md`・`Open_Questions.md`・`Resolved.md`・`Foreshadowing/_index.md`。これらは「これまで溜めた相似・仮説・未回収伏線」の累積状態であり、要約や記憶に頼らず必ずファイルから読む（読んでいないと重複登録と矛盾を書く）。
...
- [ ] **本話で言及された用語・人物・組織がすべてページ化されている**（新規作成 or 既存ページへ追記）。未ページ化の言及が残っていない
- [ ] `terminology/groups/` の帰属が最新（新語がどこにも属していない／1グループに偏在、がない）。必要ならグループの新規・分割・統合をこの時点で実施
...
- **言及時点でページ化**：用語・キャラクター（人物・組織・勢力・固有名詞）は、原文で**一度でも言及された時点でページを作成**する。情報が数行しかない場合も保留しない。既存があれば作り直さず追記・改定する（§3・§4⑤）。
- **分類グループは生き物**：`terminology/groups/` を含む階層は固定枠ではない。用語の増加に合わせて随時 追加／分割／統合／改名／再配置 し、リンクと index を追随させる（§1・§3）。
- **`analogies/` と `mysteries/` はコンテキスト圧縮後に毎回読み込む**：この2層は累積状態（相似マップ・未解決の謎・伏線）であり、記憶ではなく必ずファイルから再読する（§4①・§8.5）。
...
- **圧縮後は、`CLAUDE.md` の再確認に加えて `analogies/` と `mysteries/` を毎回必ず読み込む**（§6）。対象：
  - `wiki/analogies/recursion-map.md`
  - `wiki/mysteries/index.md` / `Open_Questions.md` / `Resolved.md` / `Foreshadowing/_index.md`

  この2層は「これまで溜めた相似・仮説・未回収伏線」の**累積状態**であり、圧縮で記憶から落ちると 同じ謎の再登録・同じ相似の重複記述・既存仮説との矛盾 が起きる。要約や記憶に頼らず、必ずファイルを読んでから追記する。
- 同様に `wiki/status.md`（進捗）も読み直し、着手範囲を確認してから作業する（§4①）。
- 同様に `backlog/`（追加機能・要望・着手中仕事の受付箱、§9）も読み、`in-progress` の件は進捗欄の最後の行から再開し、`pending` の件があれば作業に取りかかる。
...
### 8.7 関連ページ探索（graphrag・検索補助）
- **ツール**：`graphrag_tool/graphrag_search.py`（vault内・git管理・依存なし・オフライン・APIキー不要）。Obsidian の `[[WikiLink]]` をそのまま知識グラフとして流用し、BM25（エンティティ最長一致＋文字bigram トークナイザ）＋共起語拡張（RRF）＋グラフ近傍BFS で関連ページ・関連エンティティを検索する。
- **用途（発見の補助）**：§4③ で、本話の主要エンティティ（人物・用語・勢力・引喩候補）をクエリに投げ、**既存ページ・関連エンティティ・既登録の相似/謎**を把握する。重複登録・既存ページの見落とし・既知の伏線との矛盾を防ぐ。
- **位置づけ（重要）**：graphrag は**検索・発見の補助のみ**。事実判定・引喩確定の根拠には**ならない**（根拠は必ず raw 原文＋原典知識、§6）。検索結果は「候補の提示」であり、採用前に raw で裏付けを取る。
- **使い方**（vaultルートから）：
  ```bash
  PYTHONUTF8=1 python graphrag_tool/graphrag_search.py --query "アキラ 起源" --top 8 --depth 2
  PYTHONUTF8=1 python graphrag_tool/graphrag_search.py --query "..." --json   # プログラム利用
  ```
- **インデックスの管理**：`.graphrag/index.json`（gitignore 対象・ローカル生成物）にキャッシュ。スクリプトは content hash でソース変更を自動検出し、**wiki/raw が変わると次回実行時に自動再構築**（初回・変更後は数十秒）。`[index] 新規構築` が表示されたら再構築されたこと。手動で強制再構築したい場合は `.graphrag/index.json` を削除してから実行。
- **再構築の目安**：全量再構築は約15秒（1489ファイル/4801チャンク/10468エッジ、2026-09-24 実測）。1話ごとの生成では通常自動再構築の範囲で十分。

## 9. backlog/（追加機能・要望・着手中仕事の受付箱）
- **保管の対象は 2 種類**：①ユーザーから受けた**未着手の要望**、②**複数のセッションにまたがる着手中の仕事**（例：仮 CH の順次変換、ch220 までの episode 生成）。いずれも `backlog/` に 1 件 1 個の `.md` で保管する（詳細ルールは `backlog/README.md`）。
- **ファイル名**：`YYYYMMDD_<短い英語スラッグ>.md`（例：`20260926_export_api.md`）。内容は「受付日・ステータス（`pending`/`in-progress`/`done`）・指示原文（verbatim）・補足・進捗」のテンプレートを満たす。
- **着中は進捗を都度記録する**：作業が進むたびに**進捗欄に 1 行追記**（日時＋やったこと＋次の一手）。セッションがいつ切れても、このファイルを読めば続きから再開できることが目標。完了した区切り・未完了の残り・ユーザー確認待ちの点は必ず記録する。
- **毎セッション必ず確認する**：セッション開始時・コンテキスト圧縮後（§8.5）に `backlog/` を読み、`in-progress` の件は**進捗欄の最後の行から再開**し、`pending` の件は作業に取りかかる（優先順位はユーザーに確認）。
- **対応したら削除する**：完了したファイルは `backlog/` から削除し、`wiki/log.md` に 1 行追記して記録する（例：`backlog/20260926_xxx.md 対応済み・削除`）。
