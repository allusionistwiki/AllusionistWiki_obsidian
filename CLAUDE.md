...
> **①本次第の着手前に必ず `wiki/index.md` で作業状況を確認する**。進捗一覧（✅/⏳ のフォルダ別ステータス、次タスク／話一覧の状態列）で「次に生成すべき話」と各フォルダの待機範囲を特定し、その範囲に絞って作業する。確認なく推測で着手しない。
...
- **使い方**（PowerShell）：
  ...
  ```
- **既定で検出・置換する誤記**：
  - トリシーラ → トシューラ（`シ(U+30B7)ー(U+30FC)ユ(U+30E5)` が正。`セ(U+30BB)` は誤り）
  - 单词 → 単語、候选人 → 候補者、价值 → 価値、选择 → 選択
  - 简体会字（时→時・单→単・词→詞・价→価・选→選など、JIS に存在しない文字）
- **拡張**：`REPLACEMENTS` リストに `(検索, 置換, 説明)` のタプルを追加。または `custom_rules.json` を作って `--rules custom_rules.json` で指定。
- **運用フロー**：新規生成後（§4⑦）とコミット前に必ず `--scan` でクリーン確認。問題があれば `--fix` → 再スキャンでゼロを確認してからコミット。`候`・`人`・`体`・`会`・`社` は簡体と日本語で同一のため false flag（検出のみ・置換しない）。

### 8.5 コンテキスト圧縮への対応（設計書の再確認と依頼）
- **会話・文脈が長くなり、コンテキストが上限に近づくと、システム側から「コンテキスト圧縮」の依頼（通知）が出る**。その依頼に応じて直ちに本 `CLAUDE.md` を開いて設計書（schema・命名規約・鉄則・ワークフロー）を再読し、作業の前提知識を再同期する。特に §2 命名規約・§5 Lint・§6 鉄則は毎回確認。これにより「圧縮で消失した規則」の逸脱を防ぐ。
